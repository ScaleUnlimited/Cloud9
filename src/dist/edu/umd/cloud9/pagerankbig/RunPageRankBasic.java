...
public class RunPageRankBasic extends Configured implements Tool {
	...
	private static class MapWithInMapperCombiningClass extends MapReduceBase implements
		...
		private static OutputCollector<LongWritable, PageRankNode> mOutput;
	...
	private static class CombineClass extends MapReduceBase implements
			Reducer<LongWritable, PageRankNode, LongWritable, PageRankNode> {

		private static PageRankNode sIntermediateMass = new PageRankNode();

		public void reduce(LongWritable nid, Iterator<PageRankNode> values,
				OutputCollector<LongWritable, PageRankNode> output, Reporter reporter)
				throws IOException {
		...
		public void reduce(LongWritable nid, Iterator<PageRankNode> values,
			...
			while (values.hasNext()) {
				...
				if (n.getType() == PageRankNode.TYPE_STRUCTURE) {
					...
					ArrayListOfLongsWritable list = n.getAdjacenyList();
					structureReceived++;
					...
					node.setAdjacencyList(new ArrayListOfLongsWritable(arr));
				} else {
