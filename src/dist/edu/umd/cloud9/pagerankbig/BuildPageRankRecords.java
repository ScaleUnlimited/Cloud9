...
public class BuildPageRankRecords extends Configured implements Tool {
	...
	private static class MyMapper extends MapReduceBase implements
		...
		public void map(LongWritable key, Text t,
			...
			nid.set(Long.parseLong(arr[0]));
			if (arr.length == 1) {
				node.setNodeId(Long.parseLong(arr[0]));
				node.setAdjacencyList(new ArrayListOfLongsWritable());

			} else {
				node.setNodeId(Long.parseLong(arr[0]));

				long[] neighbors = new long[arr.length - 1];
				for (int i = 1; i < arr.length; i++) {
					neighbors[i - 1] = Long.parseLong(arr[i]);
				}

				node.setAdjacencyList(new ArrayListOfLongsWritable(neighbors));
			}
